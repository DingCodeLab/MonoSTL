...
from lib.losses.feature_distill_loss import compute_backbone_l1_loss,compute_backbone_weight_l1_loss
...
from lib.losses.feature_distill_loss import compute_backbone_roi_affinity_loss,compute_backbone_roi_uncertainty_affinity_loss
...
class MonoDistill(nn.Module):
    ...
    def forward(self, input,epoch, target=None):
        if self.flag == 'training' and target != None:
            ...
            rgb_pred_depth_uncertainty = rgb_outputs['depth'][:, 1:2, :, :]
            rgb_pred_depth_confidence = torch.exp(-rgb_pred_depth_uncertainty)
            student_pred_depth_confidence = _transpose_and_gather_feat(rgb_pred_depth_confidence, target['indices_all']).squeeze(-1)  # B * 1 * H * W ---> B * K
            student_confidence = student_pred_depth_confidence
            
            depth_pred_depth_uncertainty = depth_outputs['depth'][:, 1:2, :, :]
            depth_pred_depth_confidence = torch.exp(-depth_pred_depth_uncertainty)
            teacher_pred_depth_confidence = _transpose_and_gather_feat(depth_pred_depth_confidence, target['indices_all']).squeeze(-1)  # B * 1 * H * W ---> B * K
            teacher_confidence = teacher_pred_depth_confidence
            ...
            #compute_backbone_weight_l1_loss
            backbone_loss_l1 = compute_backbone_weight_l1_loss(distill_feature, depth_feat[-3:], target,teacher_confidence,student_confidence,epoch,weight_mode='student_weight')
            
            # backbone_loss_affinity = compute_backbone_resize_affinity_loss(distill_feature, depth_feat[-3:])
            
            backbone_loss_affinity = compute_backbone_roi_uncertainty_affinity_loss(distill_feature, depth_feat[-3:], target,teacher_confidence,student_confidence)
            # backbone_loss_affinity = compute_backbone_roi_affinity_loss(distill_feature, depth_feat[-3:], target)
