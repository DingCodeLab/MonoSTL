...
def inv_gaussian_map(IMAGE_HEIGHT,IMAGE_WIDTH,ratio):
    import numpy.matlib
...
def draw_umich_gaussian_torch(heatmap, center, radius, k=1):
    diameter = 2 * radius + 1
    ...
    masked_gaussian = torch.from_numpy(masked_gaussian)
    if min(masked_gaussian.shape) > 0 and min(masked_heatmap.shape) > 0:  
        torch.maximum(masked_heatmap, masked_gaussian * k, out=masked_heatmap)
    # heatmap_numpy = heatmap.detach().cpu().numpy()
    return heatmap
