...
class KITTI_Dataset(data.Dataset):
    def __init__(self, split, cfg):
        # basic configuration
        self.root_dir = '/home/XXXX/code/MonoSTL/MonoSTL_MonoDLE/data/KITTI/'
        self.split = split
        self.num_classes = 3
        self.max_objs = 50
    ...
    def __getitem__(self, item):
        ...
        # pad
        # pad = [0,self.resolution[1] - img_size[1],0,self.resolution[0] - img_size[0]]
        # img = np.pad(img, ((pad[0], pad[1]), (pad[2], pad[3]),(0,0)), 'constant',
        #                constant_values=(0, 0))
        # depth_map = np.pad(depth_map, ((pad[0], pad[1]), (pad[2], pad[3]),(0,0)), 'constant',
        #                constant_values=(0, 0))
        ...
        if self.split == 'test':
            ...
            return inputs, targets, info  # img / placeholder(fake label) / info
        ...
        depth_all = np.zeros((self.max_objs, 1), dtype=np.float32)
        ...
        indices_all = np.zeros((self.max_objs), dtype=np.int64)
...
###################################################################################################
            ...
            box2d_gt[i, :] = bbox_2d 
            indices_all[i] = center_heatmap[1] * features_size[0] + center_heatmap[0]
            depth_all[i] = objects[i].pos[-1] * aug_scale
...
###################################################################################################
        ...
        mask_2d = mask_2d.astype(np.bool8)
        mask_3d = mask_3d.astype(np.bool8)
