...
def freeze_bn(m,unfreeze_bn_layers):
    ...
    for i,k in m.named_children():
        # print(k)
        # print(i)
        if i not in unfreeze_bn_layers:
            classname = k.__class__.__name__
            if classname.find('BatchNorm2d') != -1:
                # print(k.__class__.__name__)
                k.eval()
            else:
                freeze_bn(k,unfreeze_bn_layers)
...
class Trainer(object):
    def __init__(self,
        ...
        if self.model_type == 'centernet3d_depth':
            if cfg.get('pretrain_model'):
                assert os.path.exists(cfg['pretrain_model'])
                load_checkpoint(model=self.model,
                                optimizer=None,
    ...
    def save_model(self):
        if (self.epoch % self.cfg['save_frequency']) == 0 or (self.epoch >= self.cfg['save_every_pth_after_epoch']):
            os.makedirs('log/'+self.cfg['model_save_path'], exist_ok=True)
            ckpt_name = os.path.join('log',self.cfg['model_save_path'], 'checkpoint_epoch_%d' % self.epoch)
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.cfg['max_epoch']):
            ...
            self.logger.info('lr:'+str(self.optimizer.state_dict()['param_groups'][0]['lr']))

            if self.model_type == 'centernet3d' or self.model_type == 'centernet3d_depth':
                self.train_one_epoch()

            elif self.model_type == 'distill':
                self.train_one_epoch_distill(self.epoch)
        ...
        for batch_idx, (inputs, targets, _) in enumerate(self.train_loader):
            ...
            if trained_batch % self.cfg['disp_frequency'] == 0:
                ...
                for l in avg_loss_stats:
                    avg_loss_stats[l].update(
                        rgb_stats_batch[l], inputs['rgb'].shape[0])
                    log_str += '|{} {:.4f} '.format(l, avg_loss_stats[l].avg)
                self.logger.info(log_str)

            bar.next()
        bar.finish()

    def train_one_epoch_distill(self,epoch):
        self.model.train()
        
        unfreeze_bn_layers = []
        freeze_bn(self.model.module.centernet_depth,unfreeze_bn_layers) # fix batchnorm
        self.logger.info('freeze_centernet_depth_bn')
        ...
        for batch_idx, (inputs, targets, _) in enumerate(self.train_loader):
            ...
            self.optimizer.zero_grad()
            rgb_loss, backbone_loss_l1, backbone_loss_affinity, head_loss = self.model(inputs,epoch,targets)
