...
class Tester(object):
    ...
    def test(self):
        ...
        if self.cfg['mode'] == 'single':
            ...
            self.inference()
            if self.cfg['test_mode'] != 'test_submit':
                self.evaluate()

        # test all checkpoints in the given dir
        if self.cfg['mode'] == 'all':
            checkpoints_list = []
            for _, _, files in os.walk('log/'+self.cfg['checkpoints_dir']):
                checkpoints_list = [os.path.join('log',self.cfg['checkpoints_dir'], f) for f in files if f.endswith(".pth")]
    ...
    def inference(self):
        ...
        for batch_idx, (inputs, _, info) in enumerate(self.dataloader):
            ...
            _, outputs = self.model(inputs,epoch=0)
